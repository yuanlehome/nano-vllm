...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        num_kvcache_blocks = torch.tensor(
            num_kvcache_blocks, dtype=torch.int32, pin_memory=True
        ...
        ), f"num_kvcache_blocks={config.num_kvcache_blocks} < 0, Please check the device's memory usage."
    ...
    def prepare_sampling_metadata(self, seqs: list[Sequence]) -> SamplingMetadata:
        ...
        top_ks = torch.tensor(top_ks, dtype=torch.int32, pin_memory=True).cuda(
            non_blocking=True
        )
    ...
    def run_model(
        ...
        if (
            ...
            or input_ids.size(0) > self.max_capture_size
        ):
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool) -> list[int]:
        ...
        sampling_metadata = (
            self.prepare_sampling_metadata(seqs) if self.rank == 0 else None
        )
        logits = self.run_model(input_ids, positions, is_prefill)
        token_ids = (
            self.sampler(logits, sampling_metadata).tolist() if self.rank == 0 else None
        )
        reset_context()
        return token_ids
    ...
    def capture_cudagraph(self):
        ...
        block_tables = torch.zeros(
            self.max_capture_size, max_num_blocks, dtype=torch.int32
        )
